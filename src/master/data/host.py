...
class HostsModel(Model):
    ...
    def new_host(self, name, internal_location, default):
        ...
        try:
            host_id = yield self.db.insert(
                """
                INSERT INTO `hosts`
                (`host_name`, `internal_location`, `geo_location`, `host_default`)
                VALUES (%s, %s, point(0, 0), %s)
                """, name, internal_location, int(bool(default))
            )
    ...
    @coroutine
    def update_host(self, host_id, name, internal_location, default):
        try:
            yield self.db.execute(
                """
                UPDATE `hosts`
                SET `host_name`=%s, `internal_location`=%s, `host_default`=%s
                WHERE `host_id`=%s
                """, name, internal_location, int(bool(default)), host_id
            )
        except common.database.DatabaseError as e:
